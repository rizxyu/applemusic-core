...
async function search(query) {
  const { data } = await axios.get(`https://music.apple.com/us/search?term=${encodeURIComponent(query)}`, { headers: { "Origin": "https://music.apple.com","Referer": "https://music.apple.com/", 'User-Agent': "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"}})
  const $ = cheerio.load(data)

  return [...$("div.search-tab-control.svelte-4qx79r > div.search-panel > ul > li")].map(v => $(v).find.bind($(v))).map(($) => {
