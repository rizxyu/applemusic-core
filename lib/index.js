...
async function dlAlbum(url) {
  ...
  const ids = tracks.map(result => result.id);
  const tasks = await Promise.all(ids.map(async (id) => {
      const { data: taskData } = await axios.get(`https://api.fabdl.com/apple-music/mp3-convert-task/${gid}/${id}`);
      return taskData.result
    }));
  const tids = tasks.map(task => task ? task.tid : undefined);
  const artists = tracks.map(result => result ? result.artists : undefined);
  const downloads = await Promise.all(tids.map(async (tid) => {
      try {
        const { data, headers } = await axios.get(`https://api.fabdl.com/apple-music/download-mp3/${tid}`, {
          responseType: 'arraybuffer',
        });
        ...
        console.error(`Error downloading file for tid ${tid}:`, e);
      }
    }));
}
module.exports = {
